...
import StickyScenarioSwitcher from './StickyScenarioSwitcher';
...
export default function StickyTimeline({ stages }) {
    ...
    // Dual visibility logic: hide at top (Hero) and bottom (Contact)
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (contactSection) {
                ...
                hideBottom = rect.top < windowHeight - 20;
            }

            if (firstStage) {
                const rect = firstStage.getBoundingClientRect();
                // Hide if stage-1 is still below the threshold (user in Hero)
                hideTop = rect.top > windowHeight - 100;
            }

            setShouldHide(hideBottom || hideTop);
        };
        ...
        <AnimatePresence>
            {!shouldHide && (
                <motion.div
                    ...
                    className="fixed bottom-0 left-0 right-0 z-40 glass-dark border-t border-white/10"
                >
                    <div className="px-6 md:px-12 py-3 flex items-center justify-between gap-6">
                        ...
                        {/* Scenario Switcher Integrated */}
                        <div className="hidden md:block">
                            <StickyScenarioSwitcher isIntegrated={true} />
                        </div>
                    </div>

                    {/* Mobile: Current Stage */}
                    <div className="sm:hidden pb-2 text-center border-t border-white/5 pt-2">
